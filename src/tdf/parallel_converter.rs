...
fn process_shard(
    ...
    // Batched processing to manage memory and parallelism
    const BATCH_SIZE: usize = 256;
    let range_start = partition.range.start;
    let range_end = partition.range.end;
    let total_frames = range_end - range_start;
    ...
    // Reuse a single indices buffer to avoid per-batch allocations
    let mut indices: Vec<usize> = Vec::with_capacity(BATCH_SIZE);

    // Process in chunks
    for batch_start in (range_start..range_end).step_by(BATCH_SIZE) {
        let batch_end = (batch_start + BATCH_SIZE).min(range_end);
        indices.clear();
        indices.extend(batch_start..batch_end);
        
        // Read batch of frames (uses internal parallelism for decompression)
        let frames_results = frame_reader.get_batch(&indices);
